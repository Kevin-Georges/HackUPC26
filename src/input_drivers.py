...
class TemperatureStressDriver:
    ...
    def __init__(
        ...
        for i in range(1, self._PREGENERATE):
            ...
            shock = (
                float(rng.normal(0.0, shock_std)) if rng.random() < shock_prob else 0.0
            )
            data[i] = float(
                np.clip(data[i - 1] + reversion + diffusion + shock, lo, hi)
            )
        self._data = data
...
class BinderViscosityDriver:
    ...
    def step(
        self, temperature: float, operational_load: float, maintenance_level: float
    ) -> float:
